...
2. **Authenticate (static grader credentials)**
   ...
   **Authenticate on backend link (static grader credentials)**
   ```powershell
   $authUri = "https://pwuvrbcdu3.execute-api.us-east-1.amazonaws.com/prod/authenticate"
   $tokenResponse = Invoke-RestMethod `
       -Uri $authUri `
       -Method Post `
       -ContentType "application/json" `
...
4. **Ingest a model**
  ...
  ```
   ...
   $body = '{"url":"https://huggingface.co/google-bert/bert-base-uncased"}'
   $ingestResponse = Invoke-RestMethod `
       -Uri "https://pwuvrbcdu3.execute-api.us-east-1.amazonaws.com/prod/artifact/model" `
       -Method Post `
       -Headers $headers `
       -Body $body
...
5. **Verify reads**
   ...
   Invoke-RestMethod `
       -Uri "https://pwuvrbcdu3.execute-api.us-east-1.amazonaws.com/prod/artifact/model/$id" `
       -Headers $headers

   Invoke-RestMethod `
       -Uri "https://pwuvrbcdu3.execute-api.us-east-1.amazonaws.com/prod/artifact/byName/google-bert/bert-base-uncased" `
       -Headers $headers
