...
class TestArtifactEndpoints:
    ...
    def test_get_artifact_by_name(self, mock_list, client):
    ...
    def test_get_artifact_by_id(self, mock_get, client):
        ...
        response = client.get("/artifact/model/1")
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == "1"
    ...
    @patch('src.services.artifact_storage.list_all_artifacts')
    def test_search_artifacts_by_regex(self, mock_db_list, mock_s3_list, client):
...
class TestRatingEndpoints:
    ...
    @patch('src.services.rating.run_scorer')
    def test_rate_model_by_id(self, mock_scorer, client):
...
class TestResetEndpoint:
    ...
    def test_reset_system(self, mock_clear, mock_reset, client):
...
class TestLineageEndpoint:
    ...
    def test_get_lineage(self, mock_lineage, client):
...
class TestLicenseCheckEndpoint:
    ...
    def test_license_check(self, mock_check, mock_gh, mock_model, client):
