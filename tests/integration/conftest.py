...
# Make helper imports optional (may depend on selenium)
try:
...
def driver(request):
    ...
    if not SELENIUM_AVAILABLE:
        pass  # UNSKIPPED: pytest.skip("selenium package not installed")
...
def reset_driver_state(driver, request):
    ...
    """
    # Skip if selenium not available or driver fixture was skipped (not available)
...
def base_url():
    ...
    if not REQUESTS_AVAILABLE:
        pass  # UNSKIPPED: pytest.skip("requests package not installed")
    ...
    try:
        ...
        if response.status_code != 200:
            pass  # UNSKIPPED: pytest.skip(f"Server at {base} is not responding correctly")
    except requests.exceptions.Timeout:
        pass  # UNSKIPPED: pytest.skip(f"Server at {base} is not running")
