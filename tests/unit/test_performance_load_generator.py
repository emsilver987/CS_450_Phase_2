...
class TestLoadGeneratorClass:
    ...
    def test_get_download_url(self):
        ...
        # URL format is: /artifact/model/{sanitized_model_id}/download
        assert "artifact" in url
        assert "model" in url
        assert "test_model" in url or "test/model" in url
    ...
    def test_get_download_url_sanitization(self):
        ...
        url = generator._get_download_url()
        # Check that the model_id part doesn't contain ':' (should be sanitized to '_')
        # The URL format is: /artifact/model/{sanitized_model_id}/download
        # Extract the model_id part from the URL
        if "/artifact/model/" in url:
            parts = url.split("/artifact/model/")[1].split("/download")[0]
        elif "/performance/" in url:
            parts = url.split("/performance/")[-1].split("/")[0]
        else:
            parts = url
        # The colon should be replaced with underscore
        assert ":" not in parts or ":" in url.split("?")[0]  # Allow ':' in query params (version=...)
