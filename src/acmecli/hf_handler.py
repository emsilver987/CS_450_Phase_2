...
class HFHandler:
    ...
    def _extract_hyperlinks_from_text(self, text: str) -> Dict[str, List[str]]:
        ...
        for pattern in hf_context_patterns:
            ...
            for match in re.finditer(hf_context_pattern, text, re.IGNORECASE):
                ...
                if not model_id.startswith("datasets/") and not model_id.startswith("spaces/"):
    ...
    def _categorize_url(self, url: str, links: Dict[str, List[str]]) -> None:
        ...
        if "github.com" in url_lower:
            ...
            github_match = re.search(r'github\.com/([\w\-\.]+)/([\w\-\.]+)', url_lower, re.IGNORECASE)
            if github_match:
                owner, repo = github_match.groups()
                # Filter out common false positives
                false_positives = ["github.com/explore", "github.com/about", "github.com/pricing"]
        ...
        # HuggingFace model URLs
            ...
            hf_match = re.search(r'huggingface\.co/([^/\s]+(?:/[^/\s]+)?)', url_lower, re.IGNORECASE)
            if hf_match:
    ...
    def fetch_meta(self, url: str) -> dict:
        ...
        if readme_text:
            ...
            if links["huggingface"]:
                ...
                logging.info(f"Found {len(links['huggingface'])} HuggingFace model URL(s) in README")
        ...
        try:
            ...
            with urlopen(request, timeout=10) as response:
                ...
                if html_links["github"]:
                    ...
                    logging.info(f"Found {len(html_links['github'])} GitHub URL(s) in HTML page")
