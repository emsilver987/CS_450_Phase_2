...
def get_package_metadata(pkg_name: str, version: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        response = table.get_item(
            Key={'pkg_key': f"{pkg_name}/{version}"}
        )
...
def execute_validator(script_content: str, package_data: Dict[str, Any]) -> Dict[str, Any]:
...
def log_download_event(pkg_name: str, version: str, user_id: str, 
                      status: str, reason: str = None, validation_result: Dict = None):
    """Log download event to DynamoDB"""
    try:
        table = dynamodb.Table(DOWNLOADS_TABLE)
        event_id = f"{user_id}_{pkg_name}_{version}_{datetime.now(timezone.utc).isoformat()}"
...
async def validate_package(request: ValidationRequest):
    ...
    if not package_meta:
        log_download_event(
            request.pkg_name, request.version, request.user_id,
            "blocked", "Package not found"
        )
    ...
    if not user_has_access:
        log_download_event(
            request.pkg_name, request.version, request.user_id,
            "blocked", f"User not in required groups: {allowed_groups}"
        )
    ...
    if validator_script:
        ...
        if validation_result['valid']:
            log_download_event(
                request.pkg_name, request.version, request.user_id,
                "allowed", "Validation passed", validation_result
            )
            return ValidationResponse(
                allowed=True, 
            ...
            log_download_event(
                request.pkg_name, request.version, request.user_id,
                "blocked", f"Validation failed: {validation_result.get('error', 'Unknown error')}",
                validation_result
            )
            return ValidationResponse(
                allowed=False,
        ...
        log_download_event(
            request.pkg_name, request.version, request.user_id,
            "allowed", "No validator script required"
        )
        return ValidationResponse(
            allowed=True, 
            reason="No validator script required"
        )
