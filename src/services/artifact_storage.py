...
def save_artifact(artifact_id: str, artifact_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if error_code == "ResourceNotFoundException":
            ...
            logger.error(f"Error saving artifact {artifact_id}: {error_code} - {str(e)}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error saving artifact {artifact_id}: {type(e).__name__}: {str(e)}")
        return False
...
def get_artifact(artifact_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if error_code == "ResourceNotFoundException":
            ...
            logger.error(f"Error getting artifact {artifact_id}: {error_code} - {str(e)}")
        return None
    except Exception as e:
        logger.error(f"Unexpected error getting artifact {artifact_id}: {type(e).__name__}: {str(e)}")
        return None
...
def update_artifact(artifact_id: str, updates: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if error_code == "ResourceNotFoundException":
            ...
            logger.error(f"Error updating artifact {artifact_id}: {error_code} - {str(e)}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error updating artifact {artifact_id}: {type(e).__name__}: {str(e)}")
        return False
...
def delete_artifact(artifact_id: str) -> bool:
    ...
    try:
        ...
        if error_code == "ResourceNotFoundException":
            ...
            logger.error(f"Error deleting artifact {artifact_id}: {error_code} - {str(e)}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error deleting artifact {artifact_id}: {type(e).__name__}: {str(e)}")
        return False
...
def list_all_artifacts() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Unexpected error listing artifacts: {type(e).__name__}: {str(e)}")
        return []
...
def find_models_with_null_link(link_type: str) -> List[Dict[str, Any]]:
    ...
    if link_type == "dataset":
        return [
            m for m in models
            if not m.get("dataset_id") and m.get("dataset_name")
        ]
    elif link_type == "code":
        return [
            m for m in models
            if not m.get("code_id") and m.get("code_name")
        ]
    return []
