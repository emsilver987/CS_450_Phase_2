    ...
    def rate_get(request: Request, name: str | None = None):
        ...
        if name:
            try:
                row = run_scorer(name)
                if row is None:
                    error = f"Failed to rate package '{name}': No results returned"
                else:
                    rating = {
                ...
                error = f"Failed to rate package '{name}': {str(e)}"
        ctx = {"request": request, "name": name or "", "rating": rating, "error": error}
        ...
        try:
            ...
            if file_content:
                return Response(
                    ...
                    headers={
                        "Content-Disposition": (
                            f"attachment; filename={model_id}_{version}_{component}.zip"
                        )
                    },
                )
            else:
    ...
    def license_check_get(
        ...
        if not templates:
            ...
            error_html = (
                ...
                '<title>License Check - Error</title></head>'
                '<body><h1>Frontend Error</h1>'
                '<p>Frontend templates not found. '
                'Ensure frontend/templates exists.</p></body></html>'
            )
            return Response(
                content=error_html,
        ...
        ctx = {
            ...
            "use_case": use_case or "fine-tune+inference",
            "result": result_data,
            "error": error,
        }
        try:
            ...
            # If template rendering fails, return HTML error page
            error_html = (
                f'<!DOCTYPE html><html lang="en">'
                f'<head><meta charset="utf-8">'
                f'<title>License Check - Error</title></head>'
                f'<body><h1>Template Error</h1>'
                f'<p>Template rendering failed: {str(e)}</p></body></html>'
            )
            return Response(
                content=error_html,
                media_type="text/html",
