...
(`src/middleware/jwt_auth.py`) enforces JWT verification for endpoints requiring
`X-Authorization`, while documented public routes remain open.

Highlights:

- `/authenticate` verifies admin credentials (from AWS Secrets Manager when
  `AUTH_ADMIN_SECRET_NAME` is set) and issues short-lived JWTs via
  `auth_service.create_jwt_token`, persisting each issuance with `store_token`.
  Responses are JSON of the form `{"token": "bearer …", "expires_at": …}`.
- JWTs presented through either `Authorization` or `X-Authorization` headers are
  decoded with `verify_jwt_token`; invalid or expired tokens return
  `401 {"detail": "Unauthorized"}`.
- Route handlers call `require_auth(request)` to obtain the claims and can layer
  on role checks as needed.

Resources: `src/services/auth_public.py`, `src/middleware/jwt_auth.py`,
`tests/unit/test_jwt_middleware.py`, `tests/unit/test_auth_public.py`, and
`docs/security/jwt-auth.md`.
